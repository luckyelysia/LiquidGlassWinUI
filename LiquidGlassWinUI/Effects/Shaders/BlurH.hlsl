// BlurH — 1D separable horizontal Gaussian blur.
// First pass: backdrop -> BlurH -> BlurV -> glass.
//
...
{
    float invTexel = samplerDataExt.z; // 1/width
    ...
    for (int pi = 0; pi < MAX_BLUR_PAIRS; ++pi)
    {
        ...
        float wA   = pd.y;
        sum += texture0.Sample(sampler0, float2(clampEdge(uv.x + offA), uv.y)) * wA;
        sum += texture0.Sample(sampler0, float2(clampEdge(uv.x - offA), uv.y)) * wA;

        // Pair B: offset in .z, weight in .w
        float offB = pd.z * step;
        float wB   = pd.w;
        sum += texture0.Sample(sampler0, float2(clampEdge(uv.x + offB), uv.y)) * wB;
        sum += texture0.Sample(sampler0, float2(clampEdge(uv.x - offB), uv.y)) * wB;
    }
    return sum;
}
